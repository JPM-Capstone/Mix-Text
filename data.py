...
class LabeledDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_ids = torch.tensor(np.array(input_ids.split()[1:-1] + [EOS_token], dtype=np.int64))[:128] # convert string to array
...
class UnlabeledDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        ru_input_ids, _ = (torch.tensor(np.array(ru_backtranslation['input_ids']))[:128], 
                                           torch.tensor(np.array(ru_backtranslation['attention_mask'])))
        de_input_ids, _ = (torch.tensor(np.array(de_backtranslation['input_ids']))[:128], 
